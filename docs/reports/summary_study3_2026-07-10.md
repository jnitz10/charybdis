...
**Coverage scope of the F-D null (read this before interpreting):** for SKHX and SMSN the null is measured over the FULL proxy window (2026-05-08 → 07-08) using on-disk L4 ticks. For the other eight high-funding markets it is a **recent-regime null only** — Hyperliquid REST retains just ~5,000 recent 1-minute candles (~3.5 days), so those brackets cover a single 3.5-day window. A "no clock effect" over 3.5 recent days cannot exclude a clock effect under other market conditions for those eight markets; the SKHX/SMSN full-window null is the robust part of S-D.
...
F-E: basis p95 exceeds the persistence-horizon funding edge for 57/57 twin pairs; 40/57 never exceed taker breakeven and 14/57 never exceed maker breakeven. The XMR pair `flx:XMR|hyna:XMR` has mean absolute funding-difference APR 144.85% (95% CI [121.07%, 173.22%]), half-life 0.67h ([0.40h, 1.31h]), and time above breakeven of 4.44% maker / 1.39% taker; the source report carries the near-dead-hyna venue-quality flag.

**Twin-basis is scale-invariant (units caveat — read this before comparing basis numbers):** basis = median-demeaned `ln(closeA/closeB)`, NOT the naive `(closeA−closeB)/closeB`. This matters because index twins are quoted at different unit scales/divisors — e.g. `km:US500`≈700 vs `xyz:SP500`≈7365, `km:USTECH`≈620 vs `flx:USA100`≈28729 — so a level-difference basis would report pure units artifacts (excursions of ~0.90, 9.05, even 40.9) rather than tradable dislocation. Under the log-ratio basis all 57 pairs read as real relative-price divergence, **max 3.1% across the set** (e.g. `km:US500|xyz:SP500` 0.19%, `flx:USA100|km:USTECH` 2.13%, same-scale `flx:USA500|xyz:SP500` 0.65%). F-E's "basis swamps the funding edge" holds on the corrected, scale-invariant metric.
