...
OUT = ROOT / "data/reports/ff_harvest_fills.parquet"
MARKETS = {"xyz:SKHX", "xyz:SMSN"}
...
print(f"{len(files)} trade files for {sorted(MARKETS)}", flush=True)

# per-market per-minute aggregation: minute -> [min_px, max_px, median approximated by mid of sorted sample, n]
minutes: dict[str, dict[datetime, list]] = {m: {} for m in MARKETS}
corrupt = 0
...
fee_bps = float(pl.read_parquet(ROOT / "data/reports/study3_fee_table.parquet")
                .filter(pl.col("dex") == "xyz")["effective_maker_bps"][0])
print(f"xyz effective maker fee: {fee_bps} bps", flush=True)
...
for (mkt, side, db), g in fills.partition_by(["market", "side", "delta_bps"], as_dict=True).items():
    ...
    net30 = (m30 - fee_bps) if m30 == m30 else float("nan")
